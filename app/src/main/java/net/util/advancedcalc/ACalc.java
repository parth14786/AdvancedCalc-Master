...
import net.util.advancedcalc.R;
...
public class ACalc extends Activity {
	private final char[] repChars = { '+', Constants.DIV_CHAR,
			Constants.MULT_CHAR, '^' };
	private final char[] nonClearChars = { '+', '-', Constants.DIV_CHAR,
			Constants.MULT_CHAR, '^', '!' };
	private final char[] doubles = { '.' };
	private final String[] masterClearValues = { Constants.NAN_STRING,
			Character.toString(Constants.INFINITY_CHAR) };

	private final ACalcNumberKeyListener keyL = new ACalcNumberKeyListener();
	...
	public void buttonClickListener(View v) {
		...
		// handle clearing it before, e.g. if equals was just clicked - note
		// auto-override with master clear
		if (masterClear
				|| (clearItFirst && !Helper.charInList(firstNewChar,
						nonClearChars))) {
			clearListener(v);
		}
		...
		int start = Math.min(output.getSelectionStart(),
				output.getSelectionEnd());
		int end = Math
				.max(output.getSelectionStart(), output.getSelectionEnd());
	...
	private boolean addOrReplace(char lastChar, char toAdd) {
		...
		if (Helper.charInList(lastChar, repChars)
				&& Helper.charInList(toAdd, repChars)) {
			add = false;
		}
